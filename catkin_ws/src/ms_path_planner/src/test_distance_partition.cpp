...
#include <ms_path_planner/DistancePartition.h>
...
void pointCloudCallback(const sensor_msgs::PointCloud2& pcl_msg) {
	...
	ROS_INFO("Received a new PointCloud2 message (%d points)", pcl.size());
	...
	ROS_INFO("After partition, pcl_inside has %d points, pcl_outside has %d points", pcl_inside.size(), pcl_outside.size());
