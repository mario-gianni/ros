...
protected:
	...
	std::string left_track_frame;

	std::string right_track_frame;

	std::string front_right_fl_frame;

	std::string front_left_fl_frame;

	std::string rear_right_fl_frame;

	std::string rear_left_fl_frame;
...
NiftiLaserFiltering::NiftiLaserFiltering():
	...
	has_arm(getParam<bool>(n, "has_arm", false)),
	arm_failures(0)
{
	// eps
	...
	left_track_frame = getParam<std::string>(n_, "left_track_frame", "/left_track");

	right_track_frame = getParam<std::string>(n_, "right_track_frame", "/right_track");

	front_right_fl_frame = getParam<std::string>(n_, "front_right_fl_frame", "/front_right_flipper");

	front_left_fl_frame = getParam<std::string>(n_, "front_left_fl_frame", "/front_left_flipper");

	rear_right_fl_frame = getParam<std::string>(n_, "rear_right_fl_frame", "/rear_right_flipper");

	rear_left_fl_frame = getParam<std::string>(n_, "rear_left_fl_frame", "/rear_left_flipper");
...
void NiftiLaserFiltering::robot_filter(sensor_msgs::LaserScan& scan)
{
	...
	// remove left track
	if (tf_listener.waitForTransform(laser_frame, left_track_frame, time,
			ros::Duration(1.))) {
		tf_listener.transformPointCloud(left_track_frame, close_ptcld, transformed_ptcld);
		for (unsigned int i=0; i<transformed_ptcld.points.size(); i++) {
			x = transformed_ptcld.points[i].x;
			y = transformed_ptcld.points[i].y;
			z = transformed_ptcld.points[i].z;
	...
	// remove left track
	if (tf_listener.waitForTransform(laser_frame, right_track_frame, time,
			ros::Duration(1.))) {
		tf_listener.transformPointCloud(right_track_frame, close_ptcld, transformed_ptcld);
		for (unsigned int i=0; i<transformed_ptcld.points.size(); i++) {
			x = transformed_ptcld.points[i].x;
			y = transformed_ptcld.points[i].y;
			z = transformed_ptcld.points[i].z;
	...
	if (tf_listener.waitForTransform(laser_frame, front_left_fl_frame, time,
			ros::Duration(1.))) {
		tf_listener.transformPointCloud(front_left_fl_frame, close_ptcld, transformed_ptcld);
		for (unsigned int i=0; i<transformed_ptcld.points.size(); i++) {
			x = transformed_ptcld.points[i].x;
			y = transformed_ptcld.points[i].y;
			z = transformed_ptcld.points[i].z;
	...
	if (tf_listener.waitForTransform(laser_frame, front_right_fl_frame, time,
			ros::Duration(1.))) {
		tf_listener.transformPointCloud(front_right_fl_frame, close_ptcld, transformed_ptcld);
		for (unsigned int i=0; i<transformed_ptcld.points.size(); i++) {
			x = transformed_ptcld.points[i].x;
			y = transformed_ptcld.points[i].y;
			z = transformed_ptcld.points[i].z;
	...
	if (tf_listener.waitForTransform(laser_frame, rear_left_fl_frame, time,
			ros::Duration(1.))) {
		tf_listener.transformPointCloud(rear_left_fl_frame, close_ptcld, transformed_ptcld);
		for (unsigned int i=0; i<transformed_ptcld.points.size(); i++) {
			x = transformed_ptcld.points[i].x;
			y = transformed_ptcld.points[i].y;
			z = transformed_ptcld.points[i].z;
	...
	if (tf_listener.waitForTransform(laser_frame, rear_right_fl_frame, time,
			ros::Duration(1.))) {
		tf_listener.transformPointCloud(rear_right_fl_frame, close_ptcld, transformed_ptcld);
		for (unsigned int i=0; i<transformed_ptcld.points.size(); i++) {
			x = transformed_ptcld.points[i].x;
			y = transformed_ptcld.points[i].y;
			z = transformed_ptcld.points[i].z;
